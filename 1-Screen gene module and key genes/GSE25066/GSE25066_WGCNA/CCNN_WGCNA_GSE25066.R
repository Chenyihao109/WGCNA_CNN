...
corType="pearsoon"
setwd("D:\\R_WorkSpace\\GSE25066\\GSE25066_WGCNA")
#获取表达矩阵   行是基因名  列是样本名
exp=read.csv("GSE25066_Data.csv",check.names = FALSE)
#view(exp)
...
save(datExpr,file = "exp_and_group.Rdata")
...
save(datExpr,file="exp_and_grouplist_2.Rdata")  #剔除了离群值
...
brownmodule<-GeneToColor[GeneToColor$color=="brown",]
...
for(i in 1:ncol(datExpr)){
  if(moduleColors[i] %in% c("brown")){
    choose_gene=append(choose_gene,gene_name[i])
  }
}
length(choose_gene)
table(moduleColors)
write.table(choose_gene,'brwonModule_primal_gene_list.txt',row.names = F,col.names = F,quote = F) #字符不要带引号
datExpr2=datExpr[,colnames(datExpr) %in% choose_gene]
#view(datExpr)
#不转置存起来
write.csv(datExpr2,'brown_moduel_final_expression_matrix_protein_coding_genes.csv',row.names = TRUE)
#转置后存起来
datExpr2 <- as.data.frame(t(datExpr2))
write.csv(datExpr2,'brown_moduel_final_expression_matrix_T_protein_coding_genes.csv',row.names = TRUE)
datExpr2 <- as.data.frame(t(datExpr2))
write.table(datExpr2,file="brown_module_T_matrix.txt",sep="\t",quote = FALSE,col.names = T,row.names = T)
